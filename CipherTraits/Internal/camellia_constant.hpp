...
namespace accel::Crypto::Internal {
    ...
    class CAMELLIA_CONSTANT {
    protected:

        /* SBox1110, SBox0222, SBox3033, SBox4404 are generated by the following sage script

#!/usr/bin/env sage
from sage.coding.relative_finite_field_extension import *

FA.<a> = GF(2 ^ 4, modulus = x ^ 4 + x + 1)
FB.<b> = GF(2 ^ 8, modulus = x ^ 8 + x ^ 6 + x ^ 5 + x ^ 3 + 1)

H = None
for m in Hom(FA, FB):
    if m.im_gens()[0] == b ^ 6 + b ^ 5 + b ^ 3 + b ^ 2:
        H = m
        break
assert(H != None)

FF = RelativeFiniteFieldExtension(FB, FA, H)

def transform_f(x):
    assert(0 <= x and x < 256)
    a = (x.bits() + (8 - x.nbits()) * [ 0 ])[::-1]
    b = [0] * 8

    b[1 - 1] = a[6 - 1] ^^ a[2 - 1]
    b[2 - 1] = a[7 - 1] ^^ a[1 - 1]
    b[3 - 1] = a[8 - 1] ^^ a[5 - 1] ^^ a[3 - 1]
    b[4 - 1] = a[8 - 1] ^^ a[3 - 1]
    b[5 - 1] = a[7 - 1] ^^ a[4 - 1]
    b[6 - 1] = a[5 - 1] ^^ a[2 - 1]
    b[7 - 1] = a[8 - 1] ^^ a[1 - 1]
...
def transform_g(x):
    ...
    if x == 0:
        ...
        h = h.bits()
        l = l.bits()
        t = VectorSpace(FA, 2)([ FA(l), FA(h) ])
        t = FF.absolute_field_representation(t)
        t = 1 / t
        l, h = FF.relative_field_representation(t)
        h = h.polynomial().list()
        l = l.polynomial().list()
        h = Integer(h, 2)
        l = Integer(l, 2)
...
def PrintU32Array(x, name):
    print 'static inline uint32_t %s[256] = {' % name
    for i in range(32):
        print '    ' + ', '.join([ '0x%.8x' % x[i * 8 + j] for j in range(8) ]) + (',' if i != 31 else '')
    print '};'

SBox1 = [ transform_h(transform_g(transform_f(0xc5 ^^ i))) ^^ 0x6e for i in range(256) ]
SBox2 = [ (SBox1[i] << 1 | SBox1[i] >> 7) & 0xff for i in range(256) ]
SBox3 = [ (SBox1[i] >> 1 | SBox1[i] << 7) & 0xff for i in range(256) ]
SBox4 = [ SBox1[((i << 1) | (i >> 7)) & 0xff] for i in range(256) ]

SBox1110 = [ SBox1[i] * 0x01010100 for i in range(256) ]
SBox0222 = [ SBox2[i] * 0x00010101 for i in range(256) ]
SBox3033 = [ SBox3[i] * 0x01000101 for i in range(256) ]
SBox4404 = [ SBox4[i] * 0x01010001 for i in range(256) ]

PrintU32Array(SBox1110, 'SBox1110')
PrintU32Array(SBox0222, 'SBox0222')
PrintU32Array(SBox3033, 'SBox3033')
PrintU32Array(SBox4404, 'SBox4404')

         */
        static inline uint32_t SBox1110[256] = {
        ...
        static inline uint32_t SBox0222[256] = {
        ...
        static inline uint32_t SBox3033[256] = {
        ...
        static inline uint32_t SBox4404[256] = {
        ...
        static constexpr uint64_t Sigma[] = {
            0xA09E667F3BCC908B,
            0xB67AE8584CAA73B2,
            0xC6EF372FE94F82BE,
            0x54FF53A5F1D36F1C,
