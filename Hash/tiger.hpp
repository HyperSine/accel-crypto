...
namespace accel::Hash {
    ...
    namespace Internal {
        ...
        class TIGER_CONSTANT {
        protected:
            static inline const uint64_t _t[4][256] = {
    ...
    template<size_t __Version, size_t __Bits>
    class TIGER_ALG : public Internal::TIGER_CONSTANT {
        static_assert(__Version == 1 || __Version == 2, "TIGER_ALG failure! Unsupported version.");
        static_assert(__Bits == 128 || __Bits == 160 || __Bits == 192, "TIGER_ALG failure! Unsupported bits.");
    private:

        SecureWiper<Array<uint64_t, 3>> _StateWiper;
        Array<uint64_t, 3> _State;

        template<unsigned __mul>
        ACCEL_FORCEINLINE
        void _round(uint64_t& a, uint64_t& b, uint64_t& c, const uint64_t& x) noexcept {
            const uint64_t (&t1)[256] = TIGER_CONSTANT::_t[0];
            const uint64_t (&t2)[256] = TIGER_CONSTANT::_t[1];
            const uint64_t (&t3)[256] = TIGER_CONSTANT::_t[2];
            const uint64_t (&t4)[256] = TIGER_CONSTANT::_t[3];
            
            struct QWordBytes { uint8_t byte[sizeof(uint64_t)]; }& c_ = reinterpret_cast<QWordBytes&>(c);

            c ^= x;
            a -= t1[c_.byte[0]] ^ t2[c_.byte[2]] ^ t3[c_.byte[4]] ^ t4[c_.byte[6]];
            b += t4[c_.byte[1]] ^ t3[c_.byte[3]] ^ t2[c_.byte[5]] ^ t1[c_.byte[7]];
            b *= __mul;
        }

        template<unsigned __mul>
        ACCEL_FORCEINLINE
        void _pass(uint64_t& a, uint64_t& b, uint64_t& c, const uint64_t (&x)[8]) noexcept {
            _round<__mul>(a, b, c, x[0]);
            _round<__mul>(b, c, a, x[1]);
            _round<__mul>(c, a, b, x[2]);
            _round<__mul>(a, b, c, x[3]);
            _round<__mul>(b, c, a, x[4]);
            _round<__mul>(c, a, b, x[5]);
            _round<__mul>(a, b, c, x[6]);
            _round<__mul>(b, c, a, x[7]);
        }

        ACCEL_FORCEINLINE
        void _key_schedule(uint64_t (&x)[8]) noexcept {
    ...
    public:
        ...
        void Cycle(const void* pData, size_t Rounds) noexcept {
            ...
            uint64_t c = _State[2];
            uint64_t aa, bb, cc;
            auto MessageBlock = reinterpret_cast<const uint64_t(*)[8]>(pData);
            uint64_t x[8];

            for (size_t i = 0; i < Rounds; ++i) {
                memcpy(x, MessageBlock[i], sizeof(x));
