...
namespace accel::Hash {
    ...
    private:
        ...
        /*
         * C is generated by the following sage script:
            ...
            F.<x> = GF(2 ^ 8, 'x', modulus = x ^ 8 + x ^ 4 + x ^ 3 + x ^ 2 + 1)
            MSpace = MatrixSpace(F, 8, 8)

            cir = [1, 1, 4, 1, 8, 5, 2, 9]
            MatrixC = MSpace()
            for i in range(8):
                for j in range(8):
                    MatrixC[i, j] = F.fetch_int(cir[j])
                cir = cir[-1:] + cir[:-1]

            print('static inline const uint64_t C[8][256] = {')
            for l in range(8):
                print('{')
                C = []
                for v in [ F.fetch_int(SBox[i]) for i in range(256) ]:
                    row = [ (MatrixC[l, j] * v)._log_to_int() for j in range(8) ]
                    C.append(int(binascii.hexlify(bytearray(row)[::-1]), 16))
                for i in range(0, len(C), 4):
                    print('0x%016xull, 0x%016xull, 0x%016xull, 0x%016xull, ' % (C[i], C[i + 1], C[i + 2], C[i + 3]))
                print('},')
            print('};')

         */
        static inline const uint64_t C[8][256] = {
            {
        ...
        /*
         * RoundConstant is generated by the following sage script:
            ...
            print('static inline uint64_t RoundConstant[1 + 10] = {')
            print('0x0000000000000000ull, ')
            for r in range(1, 11):
                v = bytearray([ SBox[8 * (r - 1) + j] for j in range(0, 8) ])
                print('0x%sull, ' % binascii.hexlify(v[::-1]))
            print('};')

         * */
        static inline uint64_t RoundConstant[1 + 10] = {
            0x0000000000000000ull,  // this one is just a placeholder
...
#else
        using MatrixType = Array<uint8_t, 8 * 8>;
#endif
...
#elif ACCEL_SSE2_AVAILABLE
            r[0] = _mm_xor_si128(m1[0], m2[0]);
            r[1] = _mm_xor_si128(m1[1], m2[1]);
            r[2] = _mm_xor_si128(m1[2], m2[2]);
            r[3] = _mm_xor_si128(m1[3], m2[3]);
#else
            r.AsCArray<uint64_t[8]>()[0] = m1.AsCArray<uint64_t[8]>()[0] ^ m2.AsCArray<uint64_t[8]>()[0];
            r.AsCArray<uint64_t[8]>()[1] = m1.AsCArray<uint64_t[8]>()[1] ^ m2.AsCArray<uint64_t[8]>()[1];
            r.AsCArray<uint64_t[8]>()[2] = m1.AsCArray<uint64_t[8]>()[2] ^ m2.AsCArray<uint64_t[8]>()[2];
            r.AsCArray<uint64_t[8]>()[3] = m1.AsCArray<uint64_t[8]>()[3] ^ m2.AsCArray<uint64_t[8]>()[3];
            r.AsCArray<uint64_t[8]>()[4] = m1.AsCArray<uint64_t[8]>()[4] ^ m2.AsCArray<uint64_t[8]>()[4];
            r.AsCArray<uint64_t[8]>()[5] = m1.AsCArray<uint64_t[8]>()[5] ^ m2.AsCArray<uint64_t[8]>()[5];
            r.AsCArray<uint64_t[8]>()[6] = m1.AsCArray<uint64_t[8]>()[6] ^ m2.AsCArray<uint64_t[8]>()[6];
            r.AsCArray<uint64_t[8]>()[7] = m1.AsCArray<uint64_t[8]>()[7] ^ m2.AsCArray<uint64_t[8]>()[7];
#endif
            return r;
        }
...
#elif ACCEL_SSE2_AVAILABLE
            m1[0] = _mm_xor_si128(m1[0], m2[0]);
            m1[1] = _mm_xor_si128(m1[1], m2[1]);
            m1[2] = _mm_xor_si128(m1[2], m2[2]);
            m1[3] = _mm_xor_si128(m1[3], m2[3]);
#else
            m1.AsCArray<uint64_t[8]>()[0] ^= m2.AsCArray<uint64_t[8]>()[0];
            m1.AsCArray<uint64_t[8]>()[1] ^= m2.AsCArray<uint64_t[8]>()[1];
            m1.AsCArray<uint64_t[8]>()[2] ^= m2.AsCArray<uint64_t[8]>()[2];
            m1.AsCArray<uint64_t[8]>()[3] ^= m2.AsCArray<uint64_t[8]>()[3];
            m1.AsCArray<uint64_t[8]>()[4] ^= m2.AsCArray<uint64_t[8]>()[4];
            m1.AsCArray<uint64_t[8]>()[5] ^= m2.AsCArray<uint64_t[8]>()[5];
            m1.AsCArray<uint64_t[8]>()[6] ^= m2.AsCArray<uint64_t[8]>()[6];
            m1.AsCArray<uint64_t[8]>()[7] ^= m2.AsCArray<uint64_t[8]>()[7];
#endif
        }
        ...
        static void _ThetaPiGammaTransform(MatrixType& x) noexcept {
            ...
            /*
             *              x[0] = <[00][01][02][03][04][05][06][07]><[10][11][12][13][14][15][16][17]><[20][21][22][23][24][25][26][27]><[30][31][32][33][34][35][36][37]>
             *              x[1] = <[40][41][42][43][44][45][46][47]><[50][51][52][53][54][55][56][57]><[60][61][62][63][64][65][66][67]><[70][71][72][73][74][75][76][77]>
             *
             * ->    _mm256_and_si256(x[0], _mm256_set1_epi64x(0x00000000000000FF)
             * ->    _mm256_and_si256(x[1], _mm256_set1_epi64x(0x00000000000000FF)
             *
             *                     <[00] 00  00  00  00  00  00  00 ><[10] 00  00  00  00  00  00  00 ><[20] 00  00  00  00  00  00  00 ><[30] 00  00  00  00  00  00  00 >
             *                     <[40] 00  00  00  00  00  00  00 ><[50] 00  00  00  00  00  00  00 ><[60] 00  00  00  00  00  00  00 ><[70] 00  00  00  00  00  00  00 >
             *
             * ->  _mm256_permutevar8x32_epi32(..., _mm256_set_epi32(7, 7, 7, 7, 6, 4, 2, 0))
             * ->  _mm256_permutevar8x32_epi32(..., _mm256_set_epi32(7, 7, 7, 7, 6, 4, 2, 0))
             *
             *                     <[00] 00  00  00 [10] 00  00  00 ><[20] 00  00  00 [30] 00  00  00 >< 00  00  00  00  00  00  00  00 >< 00  00  00  00  00  00  00  00 >
             *                     <[40] 00  00  00 [50] 00  00  00 ><[60] 00  00  00 [70] 00  00  00 >< 00  00  00  00  00  00  00  00 >< 00  00  00  00  00  00  00  00 >
             *
             * -> _mm256_castsi256_si128(...)
             * -> _mm256_castsi256_si128(...)
             *
             *           vindex0 = <[00] 00  00  00 [10] 00  00  00 ><[20] 00  00  00 [30] 00  00  00 >
             *           vindex1 = <[40] 00  00  00 [50] 00  00  00 ><[60] 00  00  00 [70] 00  00  00 >
             *
             * -> _mm256_i32gather_epi64(C[0], vindex0, 8)
             * -> _mm256_i32gather_epi64(C[0], vindex1, 8)
             *
             *           temp[0] = <C[0][x.As<uint8_t[8][8]>[0][0]] ><C[0][x.As<uint8_t[8][8]>[1][0]] ><C[0][x.As<uint8_t[8][8]>[2][0]] ><C[0][x.As<uint8_t[8][8]>[3][0]] >
             *           temp[1] = <C[0][x.As<uint8_t[8][8]>[4][0]] ><C[0][x.As<uint8_t[8][8]>[5][0]] ><C[0][x.As<uint8_t[8][8]>[6][0]] ><C[0][x.As<uint8_t[8][8]>[7][0]] >
             *
             */
            t0 = _mm256_and_si256(x[0], _mm256_set1_epi64x(0x00000000000000FF));
            t1 = _mm256_and_si256(x[1], _mm256_set1_epi64x(0x00000000000000FF));
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(7, 7, 7, 7, 6, 4, 2, 0));
            t1 = _mm256_permutevar8x32_epi32(t1, _mm256_set_epi32(7, 7, 7, 7, 6, 4, 2, 0));
            vindex0 = _mm256_castsi256_si128(t0);
            vindex1 = _mm256_castsi256_si128(t1);
            temp[0] = _mm256_i32gather_epi64(C[0], vindex0, 8);
            temp[1] = _mm256_i32gather_epi64(C[0], vindex1, 8);

            /*
             *              x[0] = <[00][01][02][03][04][05][06][07]><[10][11][12][13][14][15][16][17]><[20][21][22][23][24][25][26][27]><[30][31][32][33][34][35][36][37]>
             *              x[1] = <[40][41][42][43][44][45][46][47]><[50][51][52][53][54][55][56][57]><[60][61][62][63][64][65][66][67]><[70][71][72][73][74][75][76][77]>
             *
             * ->    _mm256_and_si256(x[0], _mm256_set1_epi64x(0x000000000000FF00)
             * ->    _mm256_and_si256(x[1], _mm256_set1_epi64x(0x000000000000FF00)
             *
             *                t0 = < 00 [01] 00  00  00  00  00  00 >< 00 [11] 00  00  00  00  00  00 >< 00 [21] 00  00  00  00  00  00 >< 00 [31] 00  00  00  00  00  00 >
             *                t1 = < 00 [41] 00  00  00  00  00  00 >< 00 [51] 00  00  00  00  00  00 >< 00 [61] 00  00  00  00  00  00 >< 00 [71] 00  00  00  00  00  00 >
             *
             * ->  _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(7, 7, 7, 7, 6, 4, 2, 0))
             * ->  _mm256_permutevar8x32_epi32(t1, _mm256_set_epi32(6, 4, 2, 0, 7, 7, 7, 7))
             *
             *                t0 = < 00 [01] 00  00  00 [11] 00  00 >< 00 [21] 00  00  00 [31] 00  00 >< 00  00  00  00  00  00  00  00 >< 00  00  00  00  00  00  00  00 >
             *                t1 = < 00  00  00  00  00  00  00  00 >< 00  00  00  00  00  00  00  00 >< 00 [41] 00  00  00 [51] 00  00 >< 00 [61] 00  00  00 [71] 00  00 >
             *
             * -> _mm256_blend_epi32(t0, t1, 0xF0);
             *
             *                t0 = < 00 [01] 00  00  00 [11] 00  00 >< 00 [21] 00  00  00 [31] 00  00 >< 00 [41] 00  00  00 [51] 00  00 >< 00 [61] 00  00  00 [71] 00  00 >
             *
             * -> _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(6, 5, 4, 3, 2, 1, 0, 7));
             *
             *                t0 = < 00 [71] 00  00  00 [01] 00  00 >< 00 [11] 00  00  00 [21] 00  00 >< 00 [31] 00  00  00 [41] 00  00 >< 00 [51] 00  00  00 [61] 00  00 >
             *
             * -> _mm256_bsrli_epi128(t0, 1)
             *
             *                t0 = <[71] 00  00  00 [01] 00  00  00 ><[11] 00  00  00 [21] 00  00  00 ><[31] 00  00  00 [41] 00  00  00 ><[51] 00  00  00 [61] 00  00  00 >
             *
             * -> _mm256_castsi256_si128(t0)
             * -> _mm256_extracti128_si256(t0, 1)
             *
             *           vindex0 = <[71] 00  00  00 [01] 00  00  00 ><[11] 00  00  00 [21] 00  00  00 >
             *           vindex1 = <[31] 00  00  00 [41] 00  00  00 ><[51] 00  00  00 [61] 00  00  00 >
             *
             * -> _mm256_xor_si256(temp[0], _mm256_i32gather_epi64(C[1], vindex0, 8))
             * -> _mm256_xor_si256(temp[1], _mm256_i32gather_epi64(C[1], vindex1, 8))
             *
             *           temp[0] ^= <C[1][x.As<uint8_t[8][8]>[7][1]] ><C[1][x.As<uint8_t[8][8]>[0][1]] ><C[1][x.As<uint8_t[8][8]>[1][1]] ><C[1][x.As<uint8_t[8][8]>[2][1]] >
             *           temp[1] ^= <C[1][x.As<uint8_t[8][8]>[3][1]] ><C[1][x.As<uint8_t[8][8]>[4][1]] ><C[1][x.As<uint8_t[8][8]>[5][1]] ><C[1][x.As<uint8_t[8][8]>[6][1]] >
             */
            t0 = _mm256_and_si256(x[0], _mm256_set1_epi64x(0x000000000000FF00));
            t1 = _mm256_and_si256(x[1], _mm256_set1_epi64x(0x000000000000FF00));
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(7, 7, 7, 7, 6, 4, 2, 0));
            t1 = _mm256_permutevar8x32_epi32(t1, _mm256_set_epi32(6, 4, 2, 0, 7, 7, 7, 7));
            t0 = _mm256_blend_epi32(t0, t1, 0xF0);
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(6, 5, 4, 3, 2, 1, 0, 7));
            t0 = _mm256_bsrli_epi128(t0, 1);
            vindex0 = _mm256_castsi256_si128(t0);
            vindex1 = _mm256_extracti128_si256(t0, 1);
            temp[0] = _mm256_xor_si256(temp[0], _mm256_i32gather_epi64(C[1], vindex0, 8));
            temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi64(C[1], vindex1, 8));

            t0 = _mm256_and_si256(x[0], _mm256_set1_epi64x(0x0000000000FF0000));
            t1 = _mm256_and_si256(x[1], _mm256_set1_epi64x(0x0000000000FF0000));
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(7, 7, 7, 7, 6, 4, 2, 0));
            t1 = _mm256_permutevar8x32_epi32(t1, _mm256_set_epi32(6, 4, 2, 0, 7, 7, 7, 7));
            t0 = _mm256_blend_epi32(t0, t1, 0xF0);
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(5, 4, 3, 2, 1, 0, 7, 6));
            t0 = _mm256_bsrli_epi128(t0, 2);
            vindex0 = _mm256_castsi256_si128(t0);
            vindex1 = _mm256_extracti128_si256(t0, 1);
            temp[0] = _mm256_xor_si256(temp[0], _mm256_i32gather_epi64(C[2], vindex0, 8));
            temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi64(C[2], vindex1, 8));

            t0 = _mm256_and_si256(x[0], _mm256_set1_epi64x(0x00000000FF000000));
            t1 = _mm256_and_si256(x[1], _mm256_set1_epi64x(0x00000000FF000000));
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(7, 7, 7, 7, 6, 4, 2, 0));
            t1 = _mm256_permutevar8x32_epi32(t1, _mm256_set_epi32(6, 4, 2, 0, 7, 7, 7, 7));
            t0 = _mm256_blend_epi32(t0, t1, 0xF0);
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(4, 3, 2, 1, 0, 7, 6, 5));
            t0 = _mm256_bsrli_epi128(t0, 3);
            vindex0 = _mm256_castsi256_si128(t0);
            vindex1 = _mm256_extracti128_si256(t0, 1);
            temp[0] = _mm256_xor_si256(temp[0], _mm256_i32gather_epi64(C[3], vindex0, 8));
            temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi64(C[3], vindex1, 8));

            t0 = _mm256_and_si256(x[0], _mm256_set1_epi64x(0x000000FF00000000));
            t1 = _mm256_and_si256(x[1], _mm256_set1_epi64x(0x000000FF00000000));
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(6, 6, 6, 6, 7, 5, 3, 1));
            t1 = _mm256_permutevar8x32_epi32(t1, _mm256_set_epi32(7, 5, 3, 1, 6, 6, 6, 6));
            t0 = _mm256_blend_epi32(t0, t1, 0xF0);
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(3, 2, 1, 0, 7, 6, 5, 4));
            vindex0 = _mm256_castsi256_si128(t0);
            vindex1 = _mm256_extracti128_si256(t0, 1);
            temp[0] = _mm256_xor_si256(temp[0], _mm256_i32gather_epi64(C[4], vindex0, 8));
            temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi64(C[4], vindex1, 8));

            t0 = _mm256_and_si256(x[0], _mm256_set1_epi64x(0x0000FF0000000000));
            t1 = _mm256_and_si256(x[1], _mm256_set1_epi64x(0x0000FF0000000000));
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(6, 6, 6, 6, 7, 5, 3, 1));
            t1 = _mm256_permutevar8x32_epi32(t1, _mm256_set_epi32(7, 5, 3, 1, 6, 6, 6, 6));
            t0 = _mm256_blend_epi32(t0, t1, 0xF0);
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(2, 1, 0, 7, 6, 5, 4, 3));
            t0 = _mm256_bsrli_epi128(t0, 1);
            vindex0 = _mm256_castsi256_si128(t0);
            vindex1 = _mm256_extracti128_si256(t0, 1);
            temp[0] = _mm256_xor_si256(temp[0], _mm256_i32gather_epi64(C[5], vindex0, 8));
            temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi64(C[5], vindex1, 8));

            t0 = _mm256_and_si256(x[0], _mm256_set1_epi64x(0x00FF000000000000));
            t1 = _mm256_and_si256(x[1], _mm256_set1_epi64x(0x00FF000000000000));
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(6, 6, 6, 6, 7, 5, 3, 1));
            t1 = _mm256_permutevar8x32_epi32(t1, _mm256_set_epi32(7, 5, 3, 1, 6, 6, 6, 6));
            t0 = _mm256_blend_epi32(t0, t1, 0xF0);
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(1, 0, 7, 6, 5, 4, 3, 2));
            t0 = _mm256_bsrli_epi128(t0, 2);
            vindex0 = _mm256_castsi256_si128(t0);
            vindex1 = _mm256_extracti128_si256(t0, 1);
            temp[0] = _mm256_xor_si256(temp[0], _mm256_i32gather_epi64(C[6], vindex0, 8));
            temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi64(C[6], vindex1, 8));

            t0 = _mm256_and_si256(x[0], _mm256_set1_epi64x(0xFF00000000000000));
            t1 = _mm256_and_si256(x[1], _mm256_set1_epi64x(0xFF00000000000000));
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(6, 6, 6, 6, 7, 5, 3, 1));
            t1 = _mm256_permutevar8x32_epi32(t1, _mm256_set_epi32(7, 5, 3, 1, 6, 6, 6, 6));
            t0 = _mm256_blend_epi32(t0, t1, 0xF0);
            t0 = _mm256_permutevar8x32_epi32(t0, _mm256_set_epi32(0, 7, 6, 5, 4, 3, 2, 1));
            t0 = _mm256_bsrli_epi128(t0, 3);
            vindex0 = _mm256_castsi256_si128(t0);
            vindex1 = _mm256_extracti128_si256(t0, 1);
            temp[0] = _mm256_xor_si256(temp[0], _mm256_i32gather_epi64(C[7], vindex0, 8));
            temp[1] = _mm256_xor_si256(temp[1], _mm256_i32gather_epi64(C[7], vindex1, 8));
#else
            temp.AsArrayOf<uint64_t, 8>()[0] =
                C[0][x.AsCArray<uint8_t[8][8]>()[0][0]] ^
                C[1][x.AsCArray<uint8_t[8][8]>()[7][1]] ^
                C[2][x.AsCArray<uint8_t[8][8]>()[6][2]] ^
                C[3][x.AsCArray<uint8_t[8][8]>()[5][3]] ^
                C[4][x.AsCArray<uint8_t[8][8]>()[4][4]] ^
                C[5][x.AsCArray<uint8_t[8][8]>()[3][5]] ^
                C[6][x.AsCArray<uint8_t[8][8]>()[2][6]] ^
                C[7][x.AsCArray<uint8_t[8][8]>()[1][7]];
            temp.AsArrayOf<uint64_t, 8>()[1] =
                C[0][x.AsCArray<uint8_t[8][8]>()[1][0]] ^
                C[1][x.AsCArray<uint8_t[8][8]>()[0][1]] ^
                C[2][x.AsCArray<uint8_t[8][8]>()[7][2]] ^
                C[3][x.AsCArray<uint8_t[8][8]>()[6][3]] ^
                C[4][x.AsCArray<uint8_t[8][8]>()[5][4]] ^
                C[5][x.AsCArray<uint8_t[8][8]>()[4][5]] ^
                C[6][x.AsCArray<uint8_t[8][8]>()[3][6]] ^
                C[7][x.AsCArray<uint8_t[8][8]>()[2][7]];
            temp.AsArrayOf<uint64_t, 8>()[2] =
                C[0][x.AsCArray<uint8_t[8][8]>()[2][0]] ^
                C[1][x.AsCArray<uint8_t[8][8]>()[1][1]] ^
                C[2][x.AsCArray<uint8_t[8][8]>()[0][2]] ^
                C[3][x.AsCArray<uint8_t[8][8]>()[7][3]] ^
                C[4][x.AsCArray<uint8_t[8][8]>()[6][4]] ^
                C[5][x.AsCArray<uint8_t[8][8]>()[5][5]] ^
                C[6][x.AsCArray<uint8_t[8][8]>()[4][6]] ^
                C[7][x.AsCArray<uint8_t[8][8]>()[3][7]];
            temp.AsArrayOf<uint64_t, 8>()[3] =
                C[0][x.AsCArray<uint8_t[8][8]>()[3][0]] ^
                C[1][x.AsCArray<uint8_t[8][8]>()[2][1]] ^
                C[2][x.AsCArray<uint8_t[8][8]>()[1][2]] ^
                C[3][x.AsCArray<uint8_t[8][8]>()[0][3]] ^
                C[4][x.AsCArray<uint8_t[8][8]>()[7][4]] ^
                C[5][x.AsCArray<uint8_t[8][8]>()[6][5]] ^
                C[6][x.AsCArray<uint8_t[8][8]>()[5][6]] ^
                C[7][x.AsCArray<uint8_t[8][8]>()[4][7]];
            temp.AsArrayOf<uint64_t, 8>()[4] =
                C[0][x.AsCArray<uint8_t[8][8]>()[4][0]] ^
                C[1][x.AsCArray<uint8_t[8][8]>()[3][1]] ^
                C[2][x.AsCArray<uint8_t[8][8]>()[2][2]] ^
                C[3][x.AsCArray<uint8_t[8][8]>()[1][3]] ^
                C[4][x.AsCArray<uint8_t[8][8]>()[0][4]] ^
                C[5][x.AsCArray<uint8_t[8][8]>()[7][5]] ^
                C[6][x.AsCArray<uint8_t[8][8]>()[6][6]] ^
                C[7][x.AsCArray<uint8_t[8][8]>()[5][7]];
            temp.AsArrayOf<uint64_t, 8>()[5] =
                C[0][x.AsCArray<uint8_t[8][8]>()[5][0]] ^
                C[1][x.AsCArray<uint8_t[8][8]>()[4][1]] ^
                C[2][x.AsCArray<uint8_t[8][8]>()[3][2]] ^
                C[3][x.AsCArray<uint8_t[8][8]>()[2][3]] ^
                C[4][x.AsCArray<uint8_t[8][8]>()[1][4]] ^
                C[5][x.AsCArray<uint8_t[8][8]>()[0][5]] ^
                C[6][x.AsCArray<uint8_t[8][8]>()[7][6]] ^
                C[7][x.AsCArray<uint8_t[8][8]>()[6][7]];
            temp.AsArrayOf<uint64_t, 8>()[6] =
                C[0][x.AsCArray<uint8_t[8][8]>()[6][0]] ^
                C[1][x.AsCArray<uint8_t[8][8]>()[5][1]] ^
                C[2][x.AsCArray<uint8_t[8][8]>()[4][2]] ^
                C[3][x.AsCArray<uint8_t[8][8]>()[3][3]] ^
                C[4][x.AsCArray<uint8_t[8][8]>()[2][4]] ^
                C[5][x.AsCArray<uint8_t[8][8]>()[1][5]] ^
                C[6][x.AsCArray<uint8_t[8][8]>()[0][6]] ^
                C[7][x.AsCArray<uint8_t[8][8]>()[7][7]];
            temp.AsArrayOf<uint64_t, 8>()[7] =
                C[0][x.AsCArray<uint8_t[8][8]>()[7][0]] ^
                C[1][x.AsCArray<uint8_t[8][8]>()[6][1]] ^
                C[2][x.AsCArray<uint8_t[8][8]>()[5][2]] ^
                C[3][x.AsCArray<uint8_t[8][8]>()[4][3]] ^
                C[4][x.AsCArray<uint8_t[8][8]>()[3][4]] ^
                C[5][x.AsCArray<uint8_t[8][8]>()[2][5]] ^
                C[6][x.AsCArray<uint8_t[8][8]>()[1][6]] ^
                C[7][x.AsCArray<uint8_t[8][8]>()[0][7]];
#endif
            x = temp;
        }

        ACCEL_FORCEINLINE
        static void _SigmaTransform(MatrixType& x, const MatrixType& k) noexcept {
            _MatrixXorAssign(x, k);
        }

        ACCEL_FORCEINLINE
        static void _RhoTransform(MatrixType& x, const MatrixType& k) noexcept {
            _ThetaPiGammaTransform(x);
            _SigmaTransform(x, k);
        }

        ACCEL_FORCEINLINE
        static void _KeySchedule(MatrixType& k, size_t r) noexcept {
            _ThetaPiGammaTransform(k);
            k.AsArrayOf<uint64_t, 8>()[0] ^= RoundConstant[r];
        }

        ACCEL_FORCEINLINE
        static void _WCipher(MatrixType& m, const MatrixType& key) noexcept {
            MatrixType k = key;
        ...
        SecureWiper<MatrixType> _StateWiper;
        MatrixType _State;
    public:

        WHIRLPOOL_ALG() noexcept :
            _StateWiper(_State),
            _State{} {}
        ...
        void Cycle(const void* pbMessage, size_t cRounds) noexcept {
            MatrixType MessageBlock;
            MatrixType temp;
            for (size_t i = 0; i < cRounds; ++i, pbMessage = reinterpret_cast<const char*>(pbMessage) + BlockSizeValue) {
                memcpy(MessageBlock.AsCArray(), pbMessage, BlockSizeValue);
                temp = _MatrixXor(_State, MessageBlock);
                _WCipher(MessageBlock, _State);
                _State = _MatrixXor(MessageBlock, temp);
            }
        }

        void Finish(const void* pbTail, size_t cbTail, uint64_t cbProcessed) noexcept {
            constexpr size_t cbLengthField = 256 / 8;
            static_assert(cbLengthField < BlockSizeValue);

            //
            // when [cbTail > 2 * BlockSizeValue - 1 - (256 / 8)], which is also [> BlockSize]
            // we should process more blocks so that Finish() can handle the leftover bytes.
            //
            if (cbTail > 2 * BlockSizeValue - 1 - cbLengthField) {
                size_t cRounds = cbTail / BlockSizeValue;
                Cycle(pbTail, cRounds);
                pbTail = reinterpret_cast<const uint8_t*>(pbTail) + cRounds * BlockSizeValue;
                cbTail -= cRounds * BlockSizeValue;
            }

            uint8_t FormattedTail[2 * BlockSizeValue] = {};
            uint64_t* pLengthField;
            size_t cRounds;

            memcpy(FormattedTail, pbTail, cbTail);
            FormattedTail[cbTail] = 0x80;

            cRounds = cbTail >= BlockSizeValue - cbLengthField ? 2 : 1;

            pLengthField =
                reinterpret_cast<uint64_t*>(FormattedTail + (cRounds > 1 ? (2 * BlockSizeValue - cbLengthField) :
                                                                          (BlockSizeValue - cbLengthField)));
            pLengthField[2] = ByteSwap<uint64_t>(cbProcessed >> (64 - 3));
            pLengthField[3] = ByteSwap<uint64_t>(cbProcessed << 3);

            Cycle(FormattedTail, cRounds);

            SecureWipe(FormattedTail, sizeof(FormattedTail));
        }

        Array<uint8_t, DigestSizeValue> Digest() const noexcept {
            return _State.AsArrayOf<uint8_t, 64>();
        }
    };
