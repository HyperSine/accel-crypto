    ...
    private:
        ...
        static void _ByteSub(BlockType& RefBlock) noexcept {
            for (size_t i = 0; i < BlockSizeValue; ++i)
                RefBlock.bytes[i] = SBox[RefBlock.bytes[i]];
        }

        ACCEL_FORCEINLINE
        static void _InverseByteSub(BlockType& RefBlock) noexcept {
            for (size_t i = 0; i < BlockSizeValue; ++i)
                RefBlock.bytes[i] = InverseSBox[RefBlock.bytes[i]];
        }
        ...
        static void _MixColumn(BlockType& RefBlock) noexcept {
            for (size_t i = 0; i < _Nb; ++i)
                RefBlock.dwords[i] = _MatrixTransform(RefBlock.dwords[i]);
        }

        ACCEL_FORCEINLINE
        static void _InverseMixColumn(BlockType& RefBlock) noexcept {
            for (size_t i = 0; i < _Nb; ++i)
                RefBlock.dwords[i] = _InverseMatrixTransform(RefBlock.dwords[i]);
        }
        ...
        void _InverseKeyExpansion() noexcept {
            ...
            for (size_t i = 1; i < _Nr; ++i)
                _InvKey[i] = _InverseMatrixTransform(_Key[_Nr - i]);
            _InvKey[_Nr] = _Key[0];
        }
        ...
        void _KeyExpansion(const void* pUserKey) noexcept {
            ...
            for (size_t i = _Nk; i < KeyInstance.Length(); ++i) {
                ...
                if (i % _Nk == 0) {
                    ...
                    temp.byte[0] = SBox[temp.byte[0]];
                    temp.byte[1] = SBox[temp.byte[1]];
                    temp.byte[2] = SBox[temp.byte[2]];
                    temp.byte[3] = SBox[temp.byte[3]];
                    temp.dword ^= Rcon[i / _Nk];
                }
                if (_Nk > 6 && i % _Nk == 4) {
                    temp.byte[0] = SBox[temp.byte[0]];
                    temp.byte[1] = SBox[temp.byte[1]];
